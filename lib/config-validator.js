...
/**
 * Validate and coerce a server configuration object against the module's schema.
 *
 * Performs in-place coercion and defaulting of missing properties, removes unknown properties,
 * and validates nested blocks (https, routerOptions, constraints). On failure, populates
 * `validate10.errors` with a list of error objects describing each violation.
 *
 * @param {object} data - The configuration object to validate; this object is mutated (defaults/coercions applied).
 * @param {object} [context] - Optional validation context.
 * @param {string} [context.instancePath=""] - JSON Pointer path to the validated instance (used in error objects).
 * @param {*} [context.parentData] - Parent data container (used internally for error construction).
 * @param {string|number} [context.parentDataProperty] - Property key in the parent data (used internally).
 * @param {*} [context.rootData=data] - Root-level data object.
 * @returns {boolean} `true` if the (possibly coerced) `data` conforms to the schema, `false` otherwise.
 */
